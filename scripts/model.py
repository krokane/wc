...
model.save_model("./models/xgb_goals.txt")
...
val_meta["prediction"] = model.predict(val_xgb)
...
grp = (
    val_meta[usable]
    .groupby("bucket")
    .agg(n=("rps_score", "size"), model=("rps_score", "mean"), elo=("elo_rps", "mean"))
)
...
# model deployment — train on all available data (features.csv only contains played games)
best_stopping = model.best_iteration + 1
